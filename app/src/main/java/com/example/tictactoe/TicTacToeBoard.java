...
class TicTacToeBoard extends View {
        ...
        if(action==MotionEvent.ACTION_DOWN)
        {
            ...
            if(!winningLine)
            {
                if (game.updateGameBoard(row, col)) {
                    ...
                    //updating player's turn, alternating it
    ...
    {
        canvas.drawLine(col*cellSize + (float) cellSize/2,row,cellSize*col+(float) cellSize/2,cellSize*3,paint);
    }
    ...
    {
        game.setPlayerAgainBTN(playAgain);
